...
map_shp <- district_shp %>% 
  sp::merge(nfhs5_district_estimates %>% 
              dplyr::select(district,dm_control_hba1c,dm_control_hba1c_lt7,dm_control_rbg),
            by.x="REGCODE",by.y="district",all.x=TRUE)

figB <- tm_shape(map_shp,ext=1.2) + 
  tm_fill(title= "",
          col="dm_control_hba1c_lt7",
          palette="RdYlGn",
          style = "fixed",
...
tmap_arrange(
  figA,figB,
  ncol = 2,nrow=1) %>% 
  tmap_save(.,filename=paste0(path_india_hba1c_box_folder,"/figures/national map control without restriction of 30 obs.jpg"),height=7,width=14)
